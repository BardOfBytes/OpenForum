/**
 * Footer — OpenForum Layout Component
 *
 * Minimal editorial footer with:
 * - College name and platform tagline
 * - Navigation links
 * - Social links (placeholder hrefs)
 * - Warm gray background (#eae8e2/ var(--color-surface))
 *
 * Designed as a Server Component — no client JS needed.
 */
...
const SOCIAL_LINKS = [
  ...
  {
    label: "Twitter / X",
    href: "https://x.com/openforum_csvtu",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
  ...
  {
    label: "GitHub",
    href: "https://github.com/openforum-csvtu",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path
          fillRule="evenodd"
      ...
      <div className="container-editorial py-12 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-8">
          ...
          <div className="md:col-span-3 md:col-start-8">
            <h3 className="font-body text-xs font-medium uppercase tracking-widest text-text-tertiary mb-4">
              Navigation
            </h3>
              ...
              <ul className="space-y-2.5">
                {FOOTER_NAV.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="font-body text-sm text-text-secondary hover:text-text transition-colors duration-fast"
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          <div className="md:col-span-3">
            ...
            <address className="not-italic font-body text-sm text-text-secondary leading-relaxed">
              <p>Chhattisgarh Swami Vivekanand</p>
              <p>Technical University (CSVTU)</p>
              <p className="mt-2">Bhilai, Chhattisgarh 490006</p>
            </address>
          </div>
        </div>
        ...
        <div className="mt-12 pt-6 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="font-body text-xs text-text-tertiary">
            © {new Date().getFullYear()} OpenForum · UTD CSVTU. All rights reserved.
          </p>
          <p className="font-body text-xs text-text-tertiary">
            Built with care by the student community.
          </p>
        </div>
      </div>
    </footer>
